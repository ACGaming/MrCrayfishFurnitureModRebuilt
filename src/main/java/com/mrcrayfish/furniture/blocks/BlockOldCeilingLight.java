...
public class BlockOldCeilingLight extends Block implements IPowered
{
    ...
    public BlockOldCeilingLight(Material material, boolean on, String id)
    {
        super(material);
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        if(!worldIn.isRemote)
        {
            ...
            {
                if(this == FurnitureBlocks.CEILING_LIGHT_ON_OLD)
                {
                    worldIn.setBlockState(pos, FurnitureBlocks.CEILING_LIGHT_OFF_OLD.getDefaultState(), 2);
                }
                else
                {
                    worldIn.setBlockState(pos, FurnitureBlocks.CEILING_LIGHT_ON_OLD.getDefaultState(), 2);
                }
            }
        }
        return true;
    ...
    public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos)
    {
        ...
        if(state.getValue(MODE) == Mode.REDSTONE)
        {
            ...
            {
                worldIn.setBlockState(pos, FurnitureBlocks.CEILING_LIGHT_ON_OLD.getDefaultState().withProperty(MODE, state.getValue(MODE)), 2);
            }
            else
            {
                worldIn.setBlockState(pos, FurnitureBlocks.CEILING_LIGHT_OFF_OLD.getDefaultState().withProperty(MODE, state.getValue(MODE)), 2);
            }
        }
    }
    ...
    public void setPowered(World world, BlockPos pos, boolean powered)
    {
        if(powered)
        {
            world.setBlockState(pos, FurnitureBlocks.CEILING_LIGHT_ON_OLD.getDefaultState());
        }
        else
        {
            world.setBlockState(pos, FurnitureBlocks.CEILING_LIGHT_OFF_OLD.getDefaultState());
        }
    }
