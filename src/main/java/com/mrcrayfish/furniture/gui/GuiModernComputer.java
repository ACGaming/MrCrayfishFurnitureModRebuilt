...
import com.mrcrayfish.furniture.network.message.MessageEnderShopBuy;
import com.mrcrayfish.furniture.network.message.MessageEnderShopClosed;
import com.mrcrayfish.furniture.tileentity.TileEntityModernComputer;

public class GuiModernComputer extends GuiContainer
{
    private static final ResourceLocation gui = new ResourceLocation("cfm:textures/gui/modern_computer.png");
    ...
    private TileEntityModernComputer tileEntityModernComputer;
    private RecipeData[] itemdata;

    public GuiModernComputer(InventoryPlayer inventoryplayer, TileEntityModernComputer tileEntityModernComputer)
    {
        super(new ContainerComputer(inventoryplayer, tileEntityModernComputer));
        this.tileEntityModernComputer = tileEntityModernComputer;
        this.xSize = 176;
        this.ySize = 187;
    }
    ...
    public void initGui()
    {
        ...
        this.itemNum = tileEntityModernComputer.getBrowsingInfo();
        itemdata = Recipes.getEnderShopItems();
    }
    ...
    protected void actionPerformed(GuiButton guibutton)
    {
        ...
        if(guibutton.id == 0)
        {
            ...
            this.tileEntityModernComputer.setBrowsingInfo(itemNum);
        }
        if(guibutton.id == 1)
        {
            itemNum++;
        ...
        if(guibutton.id == 2)
        {
            this.buySlot = this.tileEntityModernComputer.getStackInSlot(0);
            if(!buySlot.isEmpty())
            {
                ItemStack money = itemdata[itemNum].getCurrency();
                if(buySlot.getItem() == money.getItem() && buySlot.getItemDamage() == money.getItemDamage())
                {
                    PacketHandler.INSTANCE.sendToServer(new MessageEnderShopBuy(this.itemNum, this.tileEntityModernComputer.getPos().getX(), this.tileEntityModernComputer.getPos().getY(), this.tileEntityModernComputer.getPos().getZ()));
                }
            }
        }
    }
    ...
    {
        PacketHandler.INSTANCE.sendToServer(new MessageEnderShopClosed(tileEntityModernComputer.getPos().getX(), tileEntityModernComputer.getPos().getY(), tileEntityModernComputer.getPos().getZ()));
        super.onGuiClosed();
    }
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2)
    {
        ...
        if(itemNum < itemdata.length)
        {
            ...
            itemRender.renderItemAndEffectIntoGUI(currency, 71, 40);
            itemRender.renderItemOverlays(this.fontRenderer, currency, 71, 40);
        }
        ...
        if(itemNum < itemdata.length)
        {
            ...
            this.fontRenderer.drawString("x" + price, 88, 44, 0);
        }
